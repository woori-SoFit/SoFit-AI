...
[수정 이력]
- 매출 증가율 영향 축소: tanh 스케일링으로 극단값 억제
- 평점 분포 현실화: 2~3점 업장도 생성되도록 분포 조정
...
def generate_data(n: int = N) -> pd.DataFrame:
    ...
    has_sns = np.random.random(n) < (0.3 + lp * 0.6)
    ...
    # A7: 최장 영업 부재기간
    # [수정] 업력(일수)을 초과하지 않도록 상한선 적용
    business_age_days = business_age_months * 30
    max_inactive_raw = np.clip(
    ...
    # 업력 1개월 → scale 0.017, 12개월 → 0.2, 60개월 이상 → 1.0
    biz_age_review_scale = np.clip(business_age_months / 60.0, 0.017, 1.0)
    review_count_raw = np.clip(
        (np.random.exponential(100, n) * (0.3 + revenue_normalized * 2)
         + lp * 200).astype(int), 0, 2000
    )
    review_count = np.clip(
        (review_count_raw * biz_age_review_scale).astype(int), 0, 2000
    )
    ...
    # [수정] 매출 증가율 영향 축소: tanh 스케일링으로 극단값 억제
    # tanh(x/50) → x=50%일 때 0.76, x=100%일 때 0.96으로 수렴
    growth_momentum = (
        np.tanh(annual_revenue_growth_rate / 50) * 0.15
        + np.tanh(quarterly_revenue_growth_rate / 30) * 0.10
        + revenue_vs_industry_avg_ratio * 0.08
    )
    ...
    # 고객 신뢰도 (리뷰 평점 × 긍정 비율 — 리뷰 수 신뢰도 적용)
    review_credibility = np.log1p(review_count) / np.log1p(2000)
    high_rating_bonus = (
        np.where(review_rating >= 4.5, 0.05, 0.0) * review_credibility
    )
    customer_trust = (
    ...
    # 운영 안정성 (결제 공백 없음 + 업장 업력 마일스톤)
    biz_age_base = np.log1p(business_age_months) / np.log1p(120) * 0.05
    biz_age_milestone = (
        (business_age_months >= 60).astype(float) * 0.04
        + (business_age_months >= 120).astype(float) * 0.03
    )
    ...
    # 업주 경력 (5년, 10년 마일스톤)
    owner_exp_base = np.log1p(owner_experience_years) / np.log1p(30) * 0.04
    owner_exp_milestone = (
        (owner_experience_years >= 5).astype(float) * 0.03
        + (owner_experience_years >= 10).astype(float) * 0.03
    )
    owner_experience_bonus = owner_exp_base + owner_exp_milestone
    ...
    # 가우시안 분포 기반 S등급 변환
    # S1: 2%, S2: 5%, S3: 9%, S4: 14%, S5: 20%, S6: 20%, S7: 14%, S8: 9%, S9: 5%, S10: 2%
    gaussian_weights = np.array([0.02, 0.05, 0.09, 0.14, 0.20,
                                  0.20, 0.14, 0.09, 0.05, 0.02])
    cumulative = np.concatenate([[0], np.cumsum(gaussian_weights) * 100])
    percentiles = np.percentile(future_score, cumulative)
    grade_index = np.clip(np.digitize(future_score, percentiles[1:-1]), 0, 9)
...
if __name__ == '__main__':
    ...
    file_path = os.path.join(output_dir, 's_input_feature_40k.csv')
    ...
    print("\n[주요 변수 기술 통계]")
    print(df[[
        'avg_monthly_transaction_3m', 'online_platform_activity_index',
        'review_rating', 'annual_revenue_growth_rate', 'employee_count',
        'review_count', 'max_inactive_days',
    ]].describe().round(2))

    print("\n[수정 사항 검증]")
    solo = df[df['employee_count'] == 1]
    print(f"  직원 1명 매출 최대값: {solo['avg_monthly_transaction_3m'].max():,.0f}원 (상한 3,500만원)")
    print(f"  리뷰 평점 평균: {df['review_rating'].mean():.2f} (목표: ~3.5 이하)")
    print(f"  업력 1~3개월 업장 리뷰 수 평균: "
          f"{df[df['business_age_months'] <= 3]['review_count'].mean():.1f}건")
    print(f"  부재기간 > 업력(일) 위반 건수: "
          f"{(df['max_inactive_days'] > df['business_age_months'] * 30).sum()}건 (0이어야 정상)")
