...
| status | ENUM | 요청 처리 상태: `REQUESTED` 요청됨 / `IN_PROGRESS` 처리중 / `COMPLETED` 완료 / `FAILED` 최종 실패 |
| retry_count | INT | 재시도 횟수 (최대 3회 초과 시 FAILED 처리) |
| error_message | TEXT | 최종 실패 시 에러 메시지 (성공 시 NULL) |
