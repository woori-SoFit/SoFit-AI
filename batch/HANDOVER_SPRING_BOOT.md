...
| 기존 | 변경 후 | 비고 |
|---|---|---|
| `s_input_feature` | `s_grade_feature` | 테이블명만 변경, 컬럼 동일 |
...
### Spring Boot가 해야 할 일 (일일 배치 대상 등록)

```java
// 대출 신청 시 또는 신규 가입 시
INSERT INTO s_grade_history (user_id, feature_id, status, requested_at)
VALUES (#{userId}, #{featureId}, 'REQUESTED', NOW());
```

- `feature_id`: 해당 사용자의 `s_grade_feature` 중 가장 최근 `feature_id`를 조회하여 넣기
- `batch_execution_id`: NULL로 넣기 (Python 배치가 처리 시 채움)

---

## 3. `s_grade_feature` — Spring Boot가 INSERT하는 테이블

기존 `s_input_feature`과 동일한 구조입니다. 테이블명만 `s_grade_feature`로 변경.

My Biz Data 수집 완료 후 피처 가공 결과를 INSERT하는 로직에서 테이블명만 바꾸면 됩니다.
...
- [ ] Entity 변경: `SInputFeature` → `SGradeFeature` (테이블명만)
- [ ] Entity 변경: `SEvaluationHistory` 삭제
- [ ] Entity 변경: `ShapExplanation` → `SGradeReport` (새 구조, READ 전용)
- [ ] 대출 신청 로직: `s_grade_history`에 REQUESTED INSERT (기존 `s_calculation_request` 대체)
- [ ] 성장S등급 조회 API: `s_grade_report` JOIN `s_grade_history` 조회로 변경
- [ ] 관리자 수동 배치 API: FAILED → REQUESTED 복구 + ProcessBuilder로 Python 실행
